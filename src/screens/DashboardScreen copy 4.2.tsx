...
import GamepadController from 'react-native-gamepad-controller'; // Componente ponte do Expo
...
export const DashboardScreen: React.FC = () => {
  ...
  // Criamos uma referência para os estados atuais dos atuadores para evitar closures no callback do controle
  ...
  // 🎮 PROCESSAMENTO DOS DADOS DO CONTROLE (EXPO COMPATÍVEL)
  const handleGamepadData = (data: any) => {
    // 1. Status de Conexão
    if (data.connected !== isGamepadConnected) {
      setIsGamepadConnected(data.connected);
    }

    if (!data.connected) return;

    // 2. Analógico Esquerdo (Geralmente indexados como eixos 0 e 1 no HTML5)
    const axes = data.axes || [0, 0];
    const rawX = axes[0] || 0;
    const rawY = axes[1] || 0;

    const x = parseFloat(rawX.toFixed(2));
    const y = parseFloat((-rawY).toFixed(2)); // Inverte o Y para cima ser positivo

    const force = parseFloat(Math.min(Math.sqrt(x ** 2 + y ** 2), 1).toFixed(2));

    // Deadzone de 0.15 para evitar comandos fantasmas por folga no analógico
    if (force > 0.15) {
      handleJoystickMove({ x, y, force });
    } else if (direcao.força > 0) {
      handleJoystickStop();
    }

    // 3. Botões Pressionados (Mapeamento padrão HTML5: 0=A/X, 1=B/O, 2=X/Quadrado)
    const buttons = data.buttons || [];
    
    // Botão A (Xbox) / X (PlayStation) -> Parada de Emergência
    if (buttons[0]?.pressed) {
      handleParadaEmergencia();
    }

    // Botão X (Xbox) / Quadrado (PlayStation) -> Alterna Farol (Garante apenas um clique por gatilho)
    if (buttons[2]?.justPressed) {
      setFarol(!statesRef.current.farol);
    }

    // Botão B (Xbox) / Círculo (PlayStation) -> Alterna Buzzer
    ...
    <View style={styles.screenWrapper}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.statusRow}>
          ...
          <StatusBadge 
            label={isGamepadConnected ? "Controle Físico Conectado" : "Conectado ao ESP32"} 
            status="online" 
          />
        </View>
      ...
      {/* 🎮 PONTE INVISÍVEL PARA CAPTURAR O CONTROLE BLUETOOTH NO EXPO */}
      <GamepadController onData={handleGamepadData} />
    </View>
  );
};
